...
from torch.optim import ASGD, LBFGS, SGD
import warnings
...
class Trainer:
    """训练器类，封装模型训练逻辑（适配LBFGS的closure机制）"""
...
class Visualizer:
    ...
    def plot_learning_rate_study(self, subplot):
        ...
        for lr in learning_rates:
            print(f"\n研究学习率: {lr}")
            try:
                result = Trainer.train(SGD, f'SGD_lr_{lr}',
    ...
    def plot_epoch_study(self, subplot):
        ...
        for epochs in epoch_settings:
            print(f"\n研究训练轮数: {epochs}")
            try:
                result = Trainer.train(SGD, f'SGD_epochs_{epochs}',
                                       self.x_data, self.y_data, lr=0.01, epochs=epochs)
                if len(result['losses']) > 0:
...
def main():
    ...
    if best_result is None:
        print("没有有效的训练模型，使用SGD重新训练...")
        final_model = LinearModel()
        criterion = nn.MSELoss()
        optimizer = SGD(final_model.parameters(), lr=0.01)
