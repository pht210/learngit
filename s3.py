...
from torch.optim import Adagrad, Adamax, ASGD
import warnings
...
class Visualizer:
    ...
    def plot_learning_rate_study(x_data, y_data, subplot):
        ...
        for lr in learning_rates:
            print(f"\n研究学习率: {lr}")
            try:
                result = Trainer.train(Adagrad, f'Adagrad_lr_{lr}',
    ...
    def plot_epoch_study(x_data, y_data, subplot):
        ...
        for epochs in epoch_settings:
            print(f"\n研究训练轮数: {epochs}")
            try:
                result = Trainer.train(Adagrad, f'Adagrad_epochs_{epochs}',
                                       x_data, y_data, lr=0.1, epochs=epochs)
                if len(result['losses']) > 0:
