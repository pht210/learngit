...
from torch.optim import Adam, Rprop, ASGD
import warnings
...
class Visualizer:
    ...
    def plot_learning_rate_study(x_data, y_data, subplot):
        ...
        for lr in learning_rates:
            print(f"\n研究学习率: {lr}")
            try:
                result = Trainer.train(Adam, f'Adam_lr_{lr}',
    ...
    def plot_epoch_study(x_data, y_data, subplot):
        ...
        for epochs in epoch_settings:
            print(f"\n研究训练轮数: {epochs}")
            try:
                result = Trainer.train(Adam, f'Adam_epochs_{epochs}',
                                       x_data, y_data, lr=0.01, epochs=epochs)
                if len(result['losses']) > 0:
...
def main():
    ...
    if best_result is None:
        print("没有有效的训练模型，使用Adam重新训练...")
        final_model = LinearRegressionModel()
        criterion = nn.MSELoss()
        optimizer = Adam(final_model.parameters(), lr=0.01)
