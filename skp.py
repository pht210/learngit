...
# 5. 模型训练函数
def train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs, scaler_y, best_model_path):
    """训练模型并返回训练/测试损失记录"""
    train_losses_scaled = []
    test_losses_scaled = []
    ...
    return train_losses_scaled, test_losses_scaled, train_losses_original, test_losses_original


# 6. 结果可视化函数
def visualize_results(model, train_loader, test_loader, scaler_y, features_col, target_col,
                     num_epochs, train_losses_scaled, test_losses_scaled,
                     train_losses_original, test_losses_original, X_train_scaled, y_train_scaled):
    """生成并保存可视化结果"""
    plt.figure(figsize=(16, 14))

    # 6.1 损失曲线（双y轴：标准化+原始尺度）
...
def main():
    ...
    # 训练模型
    train_losses_scaled, test_losses_scaled, train_losses_original, test_losses_original = train_model(
        model, train_loader, test_loader, criterion, optimizer, num_epochs, scaler_y, best_model_path
    )

    # 可视化结果
    visualize_results(
        model, train_loader, test_loader, scaler_y, features_col, target_col,
        num_epochs, train_losses_scaled, test_losses_scaled,
        train_losses_original, test_losses_original, X_train_scaled, y_train_scaled
    )
