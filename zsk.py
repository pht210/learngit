...
# 4. 模型训练函数
def train_model(model, train_loader, criterion, optimizer, num_epochs, test_loader, scaler_y):
    # 记录损失
    train_losses_scaled = []
    test_losses_scaled = []
    ...
    for epoch in range(num_epochs):
        ...
        # 计算训练损失
        train_epoch_loss_scaled = train_running_loss_scaled / len(train_loader.dataset)
        ...
        # 计算测试损失
        test_epoch_loss_scaled = test_running_loss_scaled / len(test_loader.dataset)
    ...
    return (model, train_losses_scaled, test_losses_scaled,
            train_losses_original, test_losses_original, best_model_path)


# 5. 可视化函数
def visualize_results(model, train_loader, test_loader, scaler_y,
                      train_losses_scaled, test_losses_scaled,
                      train_losses_original, test_losses_original,
                      num_epochs, features_col, target_col):
    plt.figure(figsize=(16, 14))

    # 5.1 损失曲线（双y轴：标准化+原始尺度）
    ...
    ax2 = ax1.twinx()
    line3 = ax2.plot(range(1, num_epochs + 1), train_losses_original, 'g-', label='训练损失（原始）', linewidth=1,
                     alpha=0.7)
    line4 = ax2.plot(range(1, num_epochs + 1), test_losses_original, 'orange', linestyle='--', label='测试损失（原始）',
    ...
    if len(features_col) > 0:
        # 获取训练数据用于相关性计算
        X_train_scaled = np.array([x.numpy() for x, _ in train_loader.dataset])
        y_train_scaled = np.array([y.numpy()[0] for _, y in train_loader.dataset])
...
def main():
    ...
    # 训练模型
    model, train_losses_scaled, test_losses_scaled, train_losses_original, test_losses_original, best_model_path = train_model(
        model, train_loader, criterion, optimizer, num_epochs, test_loader, scaler_y
    )

    # 可视化结果
    visualize_results(
        model, train_loader, test_loader, scaler_y,
        train_losses_scaled, test_losses_scaled,
        train_losses_original, test_losses_original,
        num_epochs, features_col, target_col
    )
